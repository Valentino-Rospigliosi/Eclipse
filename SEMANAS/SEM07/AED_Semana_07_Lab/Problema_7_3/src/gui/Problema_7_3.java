...
public class Problema_7_3 extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnRemplazarPrimeraTemperaturaFebril(ActionEvent arg0) {
		int primeraTemperaturaFebril = at.posPrimeraTemperaturaFebril();
		int ultimaTemperaturaFebril = at.buscarUltimaTemperaturaFebril();

		if (primeraTemperaturaFebril != -1 && ultimaTemperaturaFebril != -1) {
			at.remplazar(primeraTemperaturaFebril, ultimaTemperaturaFebril);
			listar();
		} else {
			mensaje("No se puede reemplazar la primera temperatura febril.");
		}
	}

	protected void actionPerformedBtnRemplazarUltimaTemperaturaFebril(ActionEvent arg0) {
		int ultimaTemperaturaFebril = at.buscarUltimaTemperaturaFebril();
		double promedio = at.temperaturaPromedio();

		if (ultimaTemperaturaFebril != -1) {
			at.remplazar(ultimaTemperaturaFebril, promedio);
			listar();
		} else {
			mensaje("No se puede reemplazar la última temperatura febril.");
		}
	}
	...
	protected void actionPerformedBtnEliminarPrimeraTemperaturaFebril(ActionEvent arg0) {
		...
		if (primeraTemperaturaFebril != -1) {
			at.eliminar(primeraTemperaturaFebril);
			listar();
		} else {
			mensaje("No se puede eliminar la primera temperatura febril.");
		}
	}
