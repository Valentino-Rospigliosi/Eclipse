...
public class Propuesto_7_1 extends JFrame implements ActionListener {
	...
	private ArregloCodigos ac; // Objeto global de la clase ArregloCodigos
	...
	public Propuesto_7_1() {
		...
		ac = new ArregloCodigos(); // Inicializamos el objeto global
	}
	...
	protected void actionPerformedBtnAdicionar(ActionEvent arg0) {
		try {
			int nuevoCodigo = leerCodigo();
			ac.adicionar(nuevoCodigo);
			listar();
	...
	protected void actionPerformedBtnIntercambiar1(ActionEvent arg0) {
		if (ac.tamaño() < 2) {
			mensaje("No hay suficientes códigos para intercambiar.");
		} else {
			if (ac.intercambiarSegPen()) {
				listar();
			} else {
				mensaje("No se pueden intercambiar los códigos.");
			}
		}
	}
	...
	protected void actionPerformedBtnEliminar2(ActionEvent arg0) {
		if (ac.tamaño() > 0) {
			if (ac.eliminarCodigo()) {
				listar();
			} else {
				mensaje("No se encontró ningún código en el rango de 1000 a 1111.");
			}
		} else {
			mensaje("El arreglo está vacío.");
		}
